...
public class StudentController {
	...
	public String updateStudent(@RequestParam(name = "id") int id,@RequestParam(name = "name") String name,@RequestParam(name = "email") String email,@RequestParam(name = "gender") String gender,@RequestParam(name = "mobile") long mobile, ModelMap modelMap, HttpSession httpSession) {
		TeacherDTO signedInTeacher = (TeacherDTO) httpSession.getAttribute("teacher");
		studentService.updateStudent(id, name, email, gender, mobile);
		List<StudentDTO> students = studentService.findAllStudentsByClass(signedInTeacher);
		modelMap.addAttribute("students", students);
		modelMap.addAttribute("message", "Student Details Updated");
		return "all_students";	
	}
